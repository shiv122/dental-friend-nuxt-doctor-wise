  ...
  <div>
    <ClientOnly>
      <div class="min-h-screen p-3 md:p-10">
        ...
        >
          <Icon
            @click="router.push({ path: '/checkup/complete' })"
            class="text-white"
            height="25px"
            width="25px"
        ...
        <div v-for="(data, index) in formattedData" :key="index">
          <div class="bg-white rounded-md overflow-hidden my-5 px-4">
            ...
            <div class="md:flex md:pr-4 leading-none md:gap-5 mt-2">
              <div
                v-if="!data.no_data && data.length > 0"
                class="flex-none text-center w-full"
              >
                <img
                  @click="setViewImage(data?.image)"
                  :src="data?.image"
                  alt="pic"
            ...
            <div
              v-if="!data.no_data && data.length > 0"
              class="bg-white rounded-md overflow-hidden mt-5"
            >
                ...
                <div class="flex-col text-gray-900 flex-1 pb-10 px-0 md:px-5">
                  <Disclosure v-slot="{ open = true }" defaultOpen>
                    ...
                    <transition
                      ...
                      leave-active-class="shrink-leave-active"
                      leave-from-class="shrink-leave-from"
                          ...
                          <div class="text-md">
                            <div class="mt-5">
                              <div class="mb-2 block">
                                <div
                                  class="mb-10"
                                  v-for="(problem, ind) in data"
                                  :key="ind"
                                >
                                  <span
                                    class="font-bold text-xl mb-4 flex items-center gap-2"
                                  >
                                    <!-- {{ problem }} -->
                                    <div
                                      :style="`--p-bg:${problem.color}`"
                                      class="text-dark-600 md:w-10 md:h-10 h-6 w-6 md:text-xl text-sm rounded-full bg-[var(--p-bg)] relative flex-none"
                                    >
                                      <span
                                        class="text-white absolute inset-0 m-auto table"
                                      >
                                        {{ problem.id }}</span
                                      >
                                    </div>
                                ...
                                Youtube Reference
                                    ...
                                    <Disclosure v-slot="{ open }">
                                      ...
                                      >
                                        <DisclosurePanel
                                          class="px-4 pt-4 pb-2 text-sm text-gray-500 bg-rose-200"
                                        >
                                          <div class="flex flex-wrap gap-3">
                                            <div
                                              class="rounded-lg overflow-hidden"
                                              v-for="vid in problem.extra
                                                .youtube"
                                            >
